#define GRENADE_ACCURACY_MESSAGE_GOOD "{GREEN}完美命中目标{NORMAL}"
#define GRENADE_ACCURACY_MESSAGE_CLOSE "{LIGHT_GREEN}接近命中目标{NORMAL}"
#define GRENADE_ACCURACY_MESSAGE_FAR "{LIGHT_RED}未命中目标{NORMAL}"
#define GRENADE_ACCURACY_MESSAGE_MISSING_DATA "{LIGHT_RED}注意:{NORMAL} 投掷物 %i 数据丢失. 请在服务器控制台运行 \"sm_fixdetonations\"."
...
public void GrenadeAccuracyPrint(
  ...
  if (detonation == GrenadeAccuracyScore_GOOD) {
    if (origin <= GrenadeAccuracyScore_CLOSE) { // should this be stricter?
      PM_Message(
        client, 
        "你 " ... GRENADE_ACCURACY_MESSAGE_GOOD ... " 投掷物 %i, \"%s\".", 
        grenadeID, 
        grenadeName
      );
    } else {
      PM_Message(
        client, 
        "你 " ... GRENADE_ACCURACY_MESSAGE_GOOD ... " 投掷物 %i, \"%s\", 从 \x07不同的位置\x01.", 
        grenadeID, 
        grenadeName
      );
    }
    ...
    if (detonation == GrenadeAccuracyScore_CLOSE) {
      Format(
        prefix, 
        sizeof(prefix),
        "你 " ... GRENADE_ACCURACY_MESSAGE_CLOSE ... " 投掷物 %i, \"%s\".", 
        grenadeID, 
        grenadeName
      );
    } else {
      Format(
        prefix, 
        sizeof(prefix),
        "你 " ... GRENADE_ACCURACY_MESSAGE_FAR ... " 投掷物 %i, \"%s\".", 
        grenadeID, 
        grenadeName
      );
    }
    if (origin == GrenadeAccuracyScore_GOOD) {
      ...
      if (origin == GrenadeAccuracyScore_CLOSE) {
        PM_Message(
          client, 
          "%s 你稍微偏离了预定位置. 试着靠近瞄点, 或者输入 \".goto %i\" 到点位.",
          prefix,
          grenadeID
        );
      } else {
        ...
        PM_Message(
          client, 
          "%s 你 \x07偏离预定位置\x01. 输入 \".goto %i\" 到点位.",
          prefix,
          grenadeID
        );
      }
  ...
  if (altGrenadeID != grenadeID && altDetonation <= GrenadeAccuracyScore_CLOSE) {
    ...
    PM_Message(
        ...
        altOrigin <= GrenadeAccuracyScore_CLOSE 
          ? "你仍然 " ... GRENADE_ACCURACY_MESSAGE_GOOD ... " 投掷物 %i, \"%s\"."
          : "你仍然 " ... GRENADE_ACCURACY_MESSAGE_GOOD ... " 投掷物 %i, \"%s\", 从 \x07不同的位置\x01.", 
        altGrenadeID, 
        altGrenadeName
      );
      ...
      PM_Message(
        client, 
        altOrigin <= GrenadeAccuracyScore_CLOSE 
          ? "你仍然 " ... GRENADE_ACCURACY_MESSAGE_CLOSE ... " 投掷物 %i, \"%s\"."
          : "你仍然 " ... GRENADE_ACCURACY_MESSAGE_CLOSE ... " 投掷物 %i, \"%s\", 从 \x07不同的位置\x01.", 
        altGrenadeID, 
        altGrenadeName
      );
    }
