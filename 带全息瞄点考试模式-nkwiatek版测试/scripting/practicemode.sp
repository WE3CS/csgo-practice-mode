...
public void CheckMOTDAllowed(QueryCookie cookie, int client, ConVarQueryResult result,
  ...
  if (!StrEqual(cvarValue, "0")) {
    PrintToChat(client, "你必须设置 \x04cl_disablehtmlmotd 0 \x01才能使用它.");
  }
}
...
public void CSU_OnManagedGrenadeExplode(
  ...
  if (g_ManagedGrenadeDetonationsToFix.Size == 0) {
    ...
    if (nextPhase == GRENADE_DETONATION_FIX_PHASE_DONE) {
      PM_Message(client, "完成修复.");
    } else {
      PM_Message(client, "剩余阶段: %i.", nextPhase);
    }
  }
}